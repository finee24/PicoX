> ⚠️ **ARCHIVIATO — non più mantenuto.**
> Lo stato attivo vive ora in `context.md`, le regole stabili in `CLAUDE.md`,
> il ragionamento sui bug in `bug.md`. Questo file resta come riferimento
> storico — utile per il dettaglio forense di alcune correzioni, ma non
> affidabile sulla recency: questo stesso file conteneva sia una voce
> «✅ RISOLTO — vettore A» (rate limit su `analyze-video`, migration `0008`)
> sia, più sotto, una voce «⚠️ DA FARE — il vettore A resta aperto» che
> descrive lo stesso problema come non risolto — la seconda era quella
> vecchia, mai rimossa dopo il fix. Non fidarti dell'ordine del documento per
> capire cosa è più recente.
